...
$$W_p(\mu, \nu) = \left(\inf_{\gamma \in \Gamma(\mu,\nu)} \int_{\mathbb{R}^d \times \mathbb{R}^d} \lvert x-y\rvert^p d\gamma(x,y)\right)^{1/p}$$
...
3. Toy example: When $d=1$ and inputs are Gaussian $\mu\_i = N(m\_i, s\_i^2)$, $\nu\_j = N(m\_j, s\_j^2)$, the 1-Wasserstein distance has closed form:
    ...
    $$W_1(\mu_i, \nu_j) = \lvert m_i - m_j\rvert + \lvert s_i - s_j \rvert$$

The core difficulty is that standard kernels $k(m\_i, m\_j) = \exp(-\lvert m\_i - m\_j\rvert^2/\ell^2)$ ignore the variance terms $s\_i, s\_j$, losing crucial uncertainty information.
...
2. For 1D marginals with CDFs $F\_\mu, F\_\nu$, use the closed-form Wasserstein distance:
    ...
    $$W_p(\mu, \nu) = \left(\int_0^1 \lvert F_\mu^{-1}(q) - F_\nu^{-1}(q)\rvert^p dq\right)^{1/p}$$
...
$$\tilde{a}_\alpha = a_0 + \frac{n_G}{2}, \quad \tilde{b}_\alpha = b_0 + \frac{1}{2}E[\lvert g\rvert^2]$$
...
2. For the core tensor, substituting Gaussian likelihood and prior yields:
    ...
    $$\log q^*(g) \propto -\frac{E[\beta]}{2}\lvert y - \tilde{A}g\rvert^2 - \frac{E[\alpha]}{2}\lvert g\rvert^2$$
...
4. For hyperparameters, conjugate Gamma priors yield closed-form Gamma posteriors:
    ...
    $$\log q^*(\alpha) \propto \left(a_0 + \frac{n_G}{2} - 1\right)\log\alpha - \left(b_0 + \frac{E[\lvert g\rvert^2]}{2}\right)\alpha$$

5. The multi-mode analysis exploits the identity:

    $$\sum_{i_k=1}^{R_k} \lvert g_{i_k}^{(k)}\rvert^2 = \lvert g\rvert^2$$
...
4. Formal objective: Maximize the evidence lower bound (ELBO):
    ...
    $$\mathcal{L}_{ELBO} = \mathbb{E}_{q_\phi(v\lvert x,y)}[\log p_\theta(y\rvert x,v)] - D_{KL}[q_\phi(v\lvert x,y) \| p_\omega(v\lvert x)]$$
...
$$\{\lvert v\rvert_2^2 = 1\}^{4k}_v \mathbb{E}_{x \sim T}[\langle v, x - \theta \rangle^{2k}] \leq \frac{(1+\epsilon)^2}{1-\epsilon} \mathbb{E}_{G \sim N(0,1)}[G^{2k}]$$
...
$$\hat{\theta} = \arg\min_\theta \frac{1}{\lvert T\rvert} \sum_{(x,y) \in T} \ell(y - x^\top \theta)$$
...
4. **MODWT Energy Decomposition**: Uses the established MODWT property:
    ...
    $$\lvert X_i\rvert^2 = \sum_{j=1}^J \lvert\tilde{W}_{i,j}\rvert^2 + \lvert\tilde{V}_{i,J}\rvert^2$$
...
1. Linear operator $P: F\_S \to F\_K$ as $[Pv](s,a) = \int\_S v(s') P(ds'\lvert s,a)$
2. Policy operator $\pi: F\_K \to F\_S$ as $[\pi q](s) = \int\_A q(s,a) \pi(da\lvert s)$  
...
1. **TECHNICAL**: Assumption 8 requires universal bound $\varrho\_{F\_Q}(Pv) \leq \kappa\_P \lvert v\rvert\_{F\_S}$ - needed for majorization bounds but may be restrictive for some transition kernels
...
3. Toy example: When $d=2$, $\mu = 0$, $\sigma = I\_2$, $f(t,x,y,z) = y$, and $g(x) = \lvert x\rvert^2$, we get the heat equation $\partial\_t u + \frac{1}{2} \Delta u = u$ with $u(T,x) = \lvert x\rvert^2$. The exact solution is $u(t,x) = e^{(T-t)/2} \lvert x\rvert^2 + (T-t) e^{(T-t)/2}$, but computing this in high dimensions requires approximating conditional expectations in the BSDE.

4. Formal objective: Minimize the error

    $$\max_{0 \leq i \leq N-1} \mathbb{E}[\lvert Y_{t_i} - Y_i\rvert^2] + \mathbb{E}\left[\sum_{i=0}^{N-1} \int_{t_i}^{t_{i+1}} \lvert Z_s - Z_i\rvert^2 ds\right]$$
...
$$F_{y,i}(\theta_{y,i}) = \frac{1}{M} \sum_{m=1}^M \left\lvert S_{y,i}(p^m_i, X^m_{i+1}) - \text{NN}_{y,i}(p^m_i, \theta_{y,i})\right\rvert^2$$

Loss for $Z$-component:

$$F_{z,i}(\theta_{z,i}) = \frac{1}{M} \sum_{m=1}^M \left\lvert S_{z,i}(p^m_i, \Delta W^m_i, X^m_{i+1}) - \text{NN}_{z,i}(p^m_i, \theta_{z,i})\right\rvert^2$$
...
$$J^{HPW}_i(\theta_i) = \mathbb{E}[\lvert Y_i - Y^*_{i+1} - hf(t_i, X^\pi_i, Y_i, Z_i) + Z_i \Delta W_i\rvert^2]$$
...
1. Euler discretization error analysis using Young's inequality in the form $(a+b)^2 \leq (1+\gamma h)a^2 + (1+\frac{1}{\gamma h})b^2$ to control:
    ...
    $$\mathbb{E}[\lvert Y_{t_i} - U^m_{t_i}\rvert^2] \leq (1+Ch)\mathbb{E}[\lvert Y_{t_{i+1}} - \text{NN}_{y,i+1}\rvert^2] + Ch^2 + C\mathbb{E}\left[\int_{t_i}^{t_{i+1}} \lvert Z_s - \bar{Z}_{t_i}\rvert^2 ds\right]$$
...
3. Discrete Grönwall lemma application to obtain the final bound:
    ...
    $$\max_{0 \leq i \leq N-1} \mathbb{E}[\lvert Y_{t_i} - Y_i\rvert^2] \leq C\mathbb{E}[\lvert g(X_T) - g(X^\pi_N)\rvert^2] + Ch + C\sum_{i=0}^{N-1} \mathbb{E}[\lvert\text{approximation errors}\rvert^2]$$
...
5. The degree function satisfies $L\_1\lvert x-y\rvert \leq \lvert g(x)-g(y)\rvert$ for some $L\_1 > 0$
...
2. **MISE decomposition**: Bound the estimation error using triangle inequality:
    ...
    $$\mathbb{E}[\lvert\hat{W}_{JGS} - W\rvert_{L^2}^2] \leq 3\mathbb{E}[\lvert\hat{W}_{JGS} - W^{\text{oracle}}\rvert_{L^2}^2] + 3\mathbb{E}[\lvert W^{\text{oracle}} - W^{\text{approx}}\rvert_{L^2}^2] + 3\mathbb{E}[\lvert W^{\text{approx}} - W\rvert_{L^2}^2]$$

3. **Block-wise error control**: For each term:
   - Discretization error: $\mathbb{E}[\lvert W^{\text{approx}} - W\rvert\_{L^2}^2] \leq \frac{CL^2}{k^2}$
   - Stochastic error: $\mathbb{E}[\lvert W^{\text{oracle}} - W^{\text{approx}}\rvert\_{L^2}^2] \leq \frac{1}{4k^2}\sum\_{s,t} \mathbb{E}[\frac{1}{M\_{s,t}}]$  
...
1. Conditional transport map consistency: For numerical mediators, establish uniform convergence:
    ...
    $$\sup_{x,z} \lvert\hat{T}_j(x\rvert z_0,z_1) - T_j(x\lvert z_0,z_1)\rvert \stackrel{p}{\to} 0$$
...
$$\hat{T}_j(x\lvert z_0, \hat{T}_{Z_j}(z_0)) \stackrel{p}{\to} T_j(x\rvert z_0, T_{Z_j}(z_0))$$
...
3. Simplex transport stability: For categorical mediators, exploit continuity of optimal transport on the simplex and consistency of conditional probability estimators:
    ...
    $$\sup_{k,z} \lvert\hat{p}_{a,j}(k\rvert z) - p_{a,j}(k\lvert z)\rvert \stackrel{p}{\to} 0$$
...
5. Plug-in stability uses Lipschitz property of $\psi$ and reverse triangle inequality:
    ...
    $$\lvert U_n - \tilde{U}_n\rvert \leq 2Ln^{-1}\sum_{i=1}^n W_2(\hat{\mu}_i, \mu_i)$$
