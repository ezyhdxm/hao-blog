...
5. Grounding reward uses Hungarian algorithm for optimal bipartite matching: \\(R\_{ground} = \frac{1}{\lvert σ \rvert}\sum\_{i=1}^{\lvert σ \rvert}IoU(\hat{b\_i}, b^\*\_{σ(i)})\\)
...
3. **Adaptive ToM (A-ToM) Agent**: Maintains multiple hypothetical agents \\(\\{\\pi\_j^{(k)}\\}\_{k \\in \\{0,1,2\\}}\\) and treats ToM order estimation as an expert advice problem.
