...
Toy example application: For $X\_n = \{0, 0.5, 1\}$ with Euclidean distance and uniform weights, if learned distance $d\_n$ satisfies $\lvert d\_n(x,y) -  \rvertx-y\lvert  \rvert \leq \varepsilon\_n \to 0$, then empirical 1-mean (which is 0.5) converges to population 1-mean of uniform distribution on $[0,1]$ (which is also 0.5).
