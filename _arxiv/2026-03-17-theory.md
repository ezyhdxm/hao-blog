...
Assumptions:
1. Heavy-tailed noise: For some \\(\epsilon \in (0,1]\\), \\(E[\eta\_t | X\_{1:t}, \eta\_{1:t-1}, c\_{1:t-1}] = 0\\) and \\(E[|\eta\_t|^{1+\epsilon} | X\_{1:t}, \eta\_{1:t-1}, c\_{1:t-1}] \leq \nu\_t^{1+\epsilon}\\)
2. Bounded contexts and parameter: \\(\|x\|\_2 \leq L\\) for all \\(x \in X\_t\\) and \\(\theta^\* \in \Theta := \{\theta : \|\theta\|\_2 \leq S\}\\)
3. Corruption budget: \\(C = \sum\_{t=1}^T |c\_t|\\)
...
$$\frac{1}{2}z_t(\theta)^2 & \text{if } |z_t(\theta)| \leq \tau_t \\$$

$$\tau_t|z_t(\theta)| - \frac{\tau_t^2}{2} & \text{if } |z_t(\theta)| > \tau_t$$
...
4. OMD update with quadratic regularizer \\(\psi\_t(\theta) = \frac{1}{2}\|\theta\|\_{V\_t}^2\\):
...
Toy example application: With \\(d=2\\), coordinate arms, the corruption-dependent term in \\(\sigma\_t\\) inflates the normalization when \\(|c\_t|\\) is large, reducing the gradient magnitude and limiting corruption's influence on parameter updates.
...
The corruption term is controlled by \\(\sigma\_s \geq \sqrt{C\kappa^{-1/4}} \|X\_s\|\_{V\_{s-1}^{-1}}^{1/2}\\), giving \\(\sum\_{s=1}^t c\_s^2/\sigma\_s^2 \leq \tilde{O}(d)\\).
...
3. Apply feedback law: \\(u = -\frac{\sigma(\Phi(x) - \Phi^\*, t)}{\|\nabla \Phi(x)\|^2} \nabla \Phi(x)\\)
...
3. Spurious critical point analysis: New critical points of \\(\Phi\\) may satisfy \\(g = \beta^2 |\lambda\_{\min}| w\_{\min}\\) with \\(\lambda\_{\min} < 0\\). The Hessian of \\(\Phi\\) at such points is:
...
where \\(C(\bar{x})\\) contains curvature terms. For \\(\beta^2 < |\lambda\_{\min}|/|C(\bar{x})|\\), this remains negative, proving spurious points are saddles of \\(\Phi\\).
...
2. **Spectral approximation error**: Bound the error from approximating smooth spatial functions by truncated Laplacian eigenbasis. For signals with polynomial eigenvalue decay \\(|c\_j^{(\ell)}| \leq C \cdot j^{-\alpha/\beta}\\), this gives:
...
2. **TECHNICAL**: Polynomial decay rate assumption for spatial eigenfunctions (\\(|c\_j^{(\ell)}| \leq C \cdot j^{-\alpha/\beta}\\)) is needed for spectral approximation bounds but could potentially be relaxed.
...
5. Fractional moments at criticality: Apply Paley-Zygmund inequality with decomposition at early generation \\(\tau\_n = \lceil 2\epsilon|\log n|/\log d \rceil\\):
...
Purely theoretical. Empirical validation would involve simulating BRWs on finite Galton-Watson trees with decreasing variance functions and comparing partition function behaviors across different \\(f\\) functions. Key metrics would be convergence rates of \\(E[|W\_n^\beta - \bar{W}\_n^\beta|]\\) and verification of critical parameter invariance.
...
**Mathematical setup:** Consider a square city \\(R\\) with side length \\(|R|\\). The transit network has line density \\(\delta\_i(x,y)\\) and service headway \\(h\_i(x,y)\\) for direction \\(i \in \{E,W,N,S\}\\) at location \\((x,y) \in [0,|R|]^2\\). Vehicle flow is defined as:
...
1. **TECHNICAL**: Analysis assumes normalized gradient steps \\(z \leftarrow z + \alpha g/\|g\|\\) - needed for theoretical tractability but may not reflect all optimization scenarios in practice.
...
where \\(\Phi(x; y\_{\text{obs}}) = \frac{1}{2\sigma\_\eta^2} \|y\_{\text{obs}} - O(G(x))\|\_2^2\\).
...
1. Assume Gaussian-tail condition: \\(\sup\_x \exp(\alpha \|x\|\_H^2) \pi(x|y\_{\text{obs}}) < \infty\\) for some \\(\alpha > 0\\)
...
4. **Expected Error Estimation**: Replace worst-case bounds with expected norms \\(E\_C[K^j] = E[\|K^j \zeta\|/\|\zeta\|]\\) for Gaussian process \\(\zeta \sim GP(0, C)\\).
...
1. Establish uniform convergence \\(\sup\_{x,y \in X\_n} |d\_n(x,y) - d(x,y)| \to 0\\) a.s.
2. Verify sample fills space: \\(\sup\_{x \in X} d(x, X\_n) \to 0\\) a.s.
3. Apply main theorem

Toy example application: For \\(X\_n = \{0, 0.5, 1\}\\) with Euclidean distance and uniform weights, if learned distance \\(d\_n\\) satisfies \\(|d\_n(x,y) - |x-y|| \leq \varepsilon\_n \to 0\\), then empirical 1-mean (which is 0.5) converges to population 1-mean of uniform distribution on \\([0,1]\\) (which is also 0.5).
...
For the quadratic interaction kernel \\(W(x-y) = \frac{1}{2}\|x-y\|^2\\) with confinement \\(V(x) = \frac{\theta}{2}\|x\|^2\\), the drift simplifies to:
...
3. For small frequencies \\(wa \ll 1\\), the factor \\(\sin(wa) \approx wa\\) makes \\(|R^{(k)}\_{two}|\\) small compared to the Gaussian residual (which is exactly zero), creating spurious minima.
...
1. Restriction to quadratic interaction kernel - RESTRICTIVE (excludes many physically relevant kernels like Keller-Segel \\(W(x) = -\log\|x\|\\))
...
where \\(\beta = B - \delta\\), leading to \\(|p(x)| \leq \varepsilon\\) for appropriate choice of \\(m\\).

2. Algorithm Analysis: The correctness relies on a structural lemma showing that if \\(|v^T\mu| > \delta/2\\), then some moment of order \\(t \leq k\\) must deviate by at least \\(\varepsilon\\):
