...
2. Mathematical setup: At each round $t \in [T]$, the learner observes decision set $X\_t \subset \mathbb{R}^d$ and selects action $X\_t \in X\_t$. The environment generates stochastic reward:
    ...
    Assumptions:
    1. Heavy-tailed noise: For some $\epsilon \in (0,1]$, $E[\eta\_t \lvert X\_{1:t}, \eta\_{1:t-1}, c\_{1:t-1}] = 0$ and $E[\lvert \eta\_t \rvert^{1+\epsilon} \lvert X\_{1:t}, \eta\_{1:t-1}, c\_{1:t-1}] \leq \nu\_t^{1+\epsilon}$
...
2. Mathematical setup: Consider $N$ spatial units with $K$-dimensional binary exposure vectors $A\_i \in \{0,1\}^K$ defining $L = 2^K$ exposure combinations indexed by $\ell\_i \in \{1,\ldots,L\}$. Each unit has potential outcomes $\{Y\_{i\ell o} : \ell = 1,\ldots,L, o = 1,\ldots,O\}$ organized into tensor $\mathcal{Y} \in \mathbb{R}^{N \times L \times O}$. 
    ...
    2. Latent Ignorability: $A\_i \perp \{Y\_{i\ell o} : \ell, o\} \lvert  (Z\_i, S\_i)$  
    3. Latent Positivity: $p\_{\min} \leq P(A\_i = a(\ell) \lvert  Z\_i, S\_i) \leq 1 - p\_{\min}$
...
2. Mathematical setup: Consider episodic reinforcement learning where at timestep $t$, the agent observes history $H\_t$, samples action $A\_t \sim \pi\_\theta(\cdot \lvert  H\_t)$, and receives reward $R\_t$. The standard policy gradient forms updates:
...
1. Compute action surprisal: $\ell\_t = -\log \pi\_\theta(A\_t \lvert  H\_t)$
...
1. Assume Gaussian-tail condition: $\sup\_x \exp(\alpha \lvert x \rvert\_H^2) \pi(x\lvert y\_{\text{obs}}) < \infty$ for some $\alpha > 0$
...
3. Apply main theorem
    ...
    Toy example application: For $X\_n = \{0, 0.5, 1\}$ with Euclidean distance and uniform weights, if learned distance $d\_n$ satisfies $\lvert d\_n(x,y) -  \rvert x-y\rvert  \rvert \leq \varepsilon\_n \to 0$, then empirical 1-mean (which is 0.5) converges to population 1-mean of uniform distribution on $[0,1]$ (which is also 0.5).
...
4. Formal objective: Using Dinkelbach transformation to handle fractional objective, find:
    ...
    where $\eta\_{\theta\_a}(γ) = \sum\_{s \in \mathcal{S}} [\frac{1}{2}f^2(s) - γf(s)]\mu\_{\theta\_a}(s)$ and $\Gamma\_{\theta\_a} = \sum\_{s,s'} f(s)f(s')P\_{\theta\_a}(s'\lvert s)\mu\_{\theta\_a}(s)$.
